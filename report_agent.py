...
professional, publication-ready audit report.
  ...
  summarizer  →  drafter  →  refiner
       │              │           │
  Executive      Full report   QA pass:
  summary        in sections   consistency,
  + opinion      (Markdown)    completeness,
                               tone

Exports
-------
- report_pipeline : compiled LangGraph pipeline (ainvoke-able)
- ReportState     : TypedDict for state hand-off
"""
...
# Retry helper shared across all agents
...
Keep output under 500 words total to stay within free-tier token limits.
"""
...
async def summarizer_node(state: ReportState) -> dict:
    ...
    # Use gemini-2.5-pro here — opinion determination benefits from deeper
    # reasoning. Free tier allows 25 RPD; 1 call per audit is safe.
    llm = ChatGoogleGenerativeAI(
        model="gemini-2.5-pro",
        temperature=0.1,
        google_api_key=api_key,
        max_output_tokens=1024,  # opinion + 350-word summary fits comfortably
    )
...
Keep the total report under 2000 words to stay within free-tier output token limits.
"""
...
### 2.2 Methodology
The three-phase agentic workflow:
  - Phase 1: Document ingestion and Knowledge Graph construction (LightRAG + Gemini)
  - Phase 2: RAG-based rule retrieval, Python-driven financial analysis, gap analysis
...
*This report was generated by the Autonomous Financial Audit AI Framework.*
"""
...
async def drafter_node(state: ReportState) -> dict:
    ...
    # gemini-2.5-flash for the draft — long context, lower cost on free tier.
    # The refiner (pro) will polish it.
    llm = ChatGoogleGenerativeAI(
        model="gemini-2.5-flash",
        temperature=0.05,
        google_api_key=api_key,
        max_output_tokens=4096,  # full report needs room to breathe
    )
...
async def refiner_node(state: ReportState) -> dict:
    ...
    # Second gemini-2.5-pro call: used for final QA where accuracy matters.
    # This is the last node so using the remaining daily pro quota is justified.
    llm = ChatGoogleGenerativeAI(
        model="gemini-2.5-pro",
        temperature=0.0,
        google_api_key=api_key,
    ...
    try:
        ...
        # If pro quota is exhausted, fall back to the draft rather than failing
